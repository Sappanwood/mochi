...
export function toolExecution(input: ExecutionInput, appTools: AppTools, record: RecordExecution, signal: AbortSignal,
  ...
  const customTools: ToolDefinition[] = session.tools!.map(tool => ({
    ...
    async execute(toolCallId, args, toolSignal) {
      ...
      const request: CallbackRequest = { protocol_version: 1, app_id: run.app_id, session_id: run.session_id, run_id: run.run_id,
      ...
      try {
        ...
        if (commit) {
          if (!response.receipt || response.receipt.story_id !== scope.story_id) throw new Error('tool_transport_failed');
  ...
  return {
    ...
    beforeModel(context: unknown, contextWindow: number) {
      signal.throwIfAborted();
