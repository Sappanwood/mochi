import type { SessionMessageEntry, CustomMessageEntry } from '@earendil-works/pi-coding-agent';
...
export interface Session { session_id: string; created_at: string; system_prompt: string; thinking_level?: ThinkingLevel; tool_protocol_version?: 2; tools?: ToolSnapshot[]; tool_snapshot_hash?: string }
export interface ModelInfo { provider: string; id: string; name: string; auth: string; context_window: number; max_output_tokens: number; thinking_levels?: ThinkingLevel[] }
export interface RunInput { idempotency_key: string; provider: string; model: string; prompt: string; max_output_tokens?: number; scope?: RunScope; draft_context_digest?: string; continuation?: boolean; requested_budget?: RunBudget; budget?: RunBudget; tool_snapshot_hash?: string }
...
export interface Run {
  ...
  execution_usage?: { model_calls: number; tool_calls: number; duration_ms: number };
...
export type PiMessage = Extract<SessionMessageEntry['message'], { role: 'user' | 'assistant' | 'toolResult' }> | { role: 'custom'; customType: string; content: CustomMessageEntry['content']; display: boolean; details?: unknown; timestamp: number };
...
export interface StoredRun extends Run { app_id: string; input: RunInput; execution_started_at?: string; dispatched: boolean; events: RunEvent[]; messages?: StoredMessage[]; invocations?: ToolInvocation[] }
...
export function publicRun(run: StoredRun): Run {
  ...
  return structuredClone({ run_id, session_id, status, created_at, updated_at, result, error, usage,
    ...(run.execution_usage ? { execution_usage: run.execution_usage } : {}),
