...
已实现服务基础、应用认证、Pi 无工具适配、持久凭据与独立个人管理页面，管理服务已部署，业务 Agent API 尚未接通。
...
业务入口 readiness 保持 503；独立管理入口在取得认证目录所有权后就绪，不表示业务模型执行已可用。
...
会话与任务文件持久化至独立 Azure Files 数据共享；Queue 用于派发，任务状态为依据。具体 API、事件和错误 schema 在实现时固定。
应验证容器重启后认证可恢复、刷新后的凭据持久化、失效时提示重登录，
以及两个应用的会话隔离和客户端不暴露上游凭据。

## 待决策项

Pi 版本固定为 0.85.1；仍需验证 OpenAI 云端登录交互、实际账户可用模型、
Azure Files 上的 credential store 实测、Queue 与缩容协调，以及管理入口的 Entra 注册/路由部署。
