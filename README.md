...
v2 运行时已支持十工具、world-v1 十一工具与 materials-v1 十二工具精确快照并存。资料 OP scope 固定成员身份、模式及基线，收据逐成员核验；服务 allowlist 容量 32，单 session 仍最多 16 工具/32 KiB。业务数据由 Write 提交。此增量未发布，部署顺序为兼容 runtime、CCP 精确配置、Write 新入口；见 [应用工具](docs/APP_TOOLS.md#单故事资料工具本地实现)。
