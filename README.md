...
- [应用工具](docs/APP_TOOLS.md)：工具配置、快照、授权边界、回调与收据核实。
...
| `MOCHI_APP_TOOLS` | 可选 app 到 HTTPS endpoint/audience/工具 allowlist 的 JSON 配置，见应用工具契约；省略保持无工具 |
...
默认无工具对话；应用可显式启用登记的工具快照。工具业务端需实现反向 Entra 身份验证，生产配置不接受明文 HTTP 或免认证。provider 限定：
...
纯文档变更完成文档检查即可，不触发发布。日常镜像发布仍执行上文的停机、备份与 owner 交接。
...
v2 运行时已支持十工具、world-v1 十一工具与 materials-v1 十二工具精确快照并存。资料 OP scope 固定成员身份、模式及基线，收据逐成员核验；服务 allowlist 容量 32，单 session 仍最多 16 工具/32 KiB。业务数据由 Write 提交。此增量已按兼容 runtime、CCP 精确配置、Write 新入口的顺序发布，云端保留 19 条工具版本配置；见 [应用工具](docs/APP_TOOLS.md#单故事资料工具)。
